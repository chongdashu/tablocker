...
async def sync_blocklist(
    ...
    current_user: PayingUser = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    logger.info(f"Syncing blocklist for user {current_user.id}")
    # Get existing patterns
    existing_patterns = db.query(BlockedPattern).filter(BlockedPattern.supabase_user_id == current_user.id).all()
    existing_pattern_set = {p.pattern for p in existing_patterns}
    logger.info(f"Found {len(existing_patterns)} existing patterns for user {current_user.id}")
    ...
    for pattern in request.patterns:
        if pattern.pattern not in existing_pattern_set:
            new_pattern = BlockedPattern(
                user_id=current_user.id, pattern=pattern.pattern, created_at=pattern.created_at
            )
            db.add(new_pattern)
            new_patterns_count += 1

    logger.info(f"Adding {new_patterns_count} new patterns for user {current_user.id}")
    db.commit()

    # Fetch updated patterns
    updated_patterns = db.query(BlockedPattern).filter(BlockedPattern.supabase_user_id == current_user.id).all()
    response_patterns = [
        BlockedPatternSchema(pattern=p.pattern, created_at=p.created_at.isoformat()) for p in updated_patterns
    ]
    logger.info(f"Returning {len(response_patterns)} total patterns for user {current_user.id}")
